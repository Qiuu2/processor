...
**该领域未找到**:LOFAR/DEMON 被动声呐线谱检测无可用开源;SOGI-FLL / Regalia 格型 ANF 无可商用成熟 C 实现;音频域自动去嗡无"可商用+自动检测"实现(好的全是 GPL:Audacity/LSP/x42,或闭源 iZotope RX)。
**⚠ 规划陷阱**:`liquid-dsp`(MIT 已核)**未能确认其存在 notch API**(头文件抓取被截断)——**不得按"它有 notch"做方案规划**,要用先自核。`michaellass/ladspa-notch-harmonics` license 未核(前路称 MIT,本路未核,存疑)。
...
### 1.10 自研/移植工作量估算 —— ⚠ **已降级,原估算偏乐观(lead 2026-07-31 自我修正)**

> **修正声明**:下表 "~50% 可省" 系调查员估算 [L4],lead 初次转述时**未加压力测试即传播**。经核算参数尺度后判定**偏乐观,降级为"待 W1 拿实际代码复核"**。修正依据(lead 手算 [L3],见下)。**排期不得引用此数字**。

**为什么偏乐观 —— 三条硬约束**(`分辨率 = fs/N`,lead 核算):

| 场景 | fs | N | 分辨率 | 分析窗 |
|---|---|---|---|---|
| PX4 典型(陀螺) | 1 kHz | 256 | **3.9 Hz** | 256 ms |
| **我们照搬 N=256** | 48 kHz | 256 | **187 Hz ❌ 不可用** | 5.3 ms |
| 我们 N=2048 | 48 kHz | 2048 | 23.4 Hz | 42.7 ms |
| 我们 N=4096 | 48 kHz | 4096 | 11.7 Hz | 85.3 ms |

1. **参数一个都不能抄**:采样率高 48×,FFT 须涨到 2048-4096 → 单次 FFT 算力 ×8-16,再 ×8 通道。**这是 D13 必算的账**,不是"移植"能绕开的。
2. **陷波比 FFT 频点还窄**:1/10 oct 在 100Hz 处仅 **6.9 Hz** 宽,而 N=2048 分辨率 23.4 Hz → **靠 bin 索引无法对准陷波**。→ **Quinn 第二估计器(bin 内插)从"可选优化"升级为"必需件"**;这反而是 PX4 最值钱的一处(避开抛物线内插精度不足、Jacobsen 加窗有偏等已知坑)。
3. **移植的是设计不是代码**:真正省的是**设计与调试轮次**(架构已被几十万台设备验证)+ 几个非显然技巧(Quinn 内插、0.25 bin 迟滞豁免、0.1Hz 才重算系数防抖、7 点中值);**参数/判断逻辑/深度控制/定点化全部自研**。

**架构利好(lead 核算新增)**:检测是**旁链(side-chain)**,不在音频通路 → **43-85ms 分析窗不占 <12ms 延迟预算**,只影响反应时间(对标 Sabine 0.4s 抓一次啸叫,足够)。8 通道可**轮流分析**(每帧一路,8 帧一轮 = 10.7ms@64样本帧),不必每通道每帧都算。**待评估选项:检测旁链抽取到 16kHz**(啸叫主要在 200Hz-8kHz)→ N=1024 得 15.6Hz 分辨率、算力降至 1/4。以上均 [L3 手算],交 W1/D13 定案。
