 ...
 *   选它的唯一理由是**定点链内电平**。
 *   ⛔ 依据**不是**「省 31.14 dB」—— 那是两臂之【差值】,不是余量(critic D3D4 MAJOR-2)。
 *   ✅ 依据 = **首个越界的合法配置**(满刻度 + 2 段 +15 dB 同频 @40 Hz,LR4 HP @120 Hz)下:
 *      ⛔ 刻意不写「最坏」——3 段 ⇒ +45.00 dBFS,8 段更高;论证要的是【越界门槛】。
 *      PEQ 在前最大节点 **+30.00 dBFS** ⇒ 越 Q4.27 链内余量 24.08 dB;
 *      分频在前 **−8.28 dBFS** ⇒ 不越。[设计件 §2③ / d34_chain/check_r16.py]
 * ⚠ **LR 分频的求和极性由【滤波器阶数 N】决定,⛔ 不是 dB/oct**:
 *   N = xo_slope ÷ 6 ∈ {2,4,6,8};**N mod 4 == 2 ⇒ 须反相**(⇒ 12 与 36 dB/oct 须反相)。
 *   由 `chdsp_xover_needs_polarity_flip()` 给出;写反 ⇒ 分频点出现**零点**(理想深度 −∞)。
 *   ⛔ 原写「87.72 dB 深谷」**已撤回**:那是网格伪影,不是物理量(critic D3D4 erratum2)。
 */
