 ...
 *   ⇒ LR2 / LR6 在分频点出现**零点**(理想系数下深度 = −∞)。
 *   ⛔ 原写「实测 65.86 / 56.32 dB」**已撤回**:那是网格伪影,不是物理量
 *      (critic D3D4 erratum2;同族的 87.72 于 2026-08-05 一并撤回)。
 *      判别法:把网格改一倍,数会变而没写明 ⇒ 它是【器械】的性质,不是【被测物】的。
 *   ✅ 设计件侧已于 2026-08-05 整改(critic MAJOR-4):§4③ 表头改为 dB/oct 并单列
 *      「LR 滤波器阶数 N = xo_slope ÷ 6」一行;规则改写为「N mod 4 == 2 ⇒ 须反相」。
 *      ⇒ 本段描述的是**当时**的设计件状态,保留为留痕。
 *
 * ⚠ 实证(本文件改动前):
