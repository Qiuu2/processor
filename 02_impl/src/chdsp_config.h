...
/**
 * ✅ **待定项 ② 已闭(2026-08-04,r10)**:PRD §二.5 的**三种工作模式**已落地。
 * ⚠ 原状态是「只有形状,没有行为」—— `notch_mode[]` 在 init 里被赋值,而**全库零消费者**
 *   (D6-ao 接线审计)⇒ 三种模式实际只有一种(全动态)。
 * ⇒ 现由 `chdsp_notch.h/.c` 的**槽位簿记**实现:FIXED / DYNAMIC / HYBRID 三种语义
 *   各自有可穷举的行为(CHK-N1…N5),⛔ 不再是一个枚举。
 * ⚠ **但三种模式的【语义】PRD 未定义** —— 下面的定义是按工程惯例推的,
