...
**状态:⛔ 检查点 r10 —— 本轮到此,已报 lead 打 tag。**
本轮内容 = critic 未派条目全闭(MAJOR-3/5/6、m-1…m-11)+ `butter_q` 显眼留痕 + **陷波器组三种工作模式落地**。

**基线(2026-08-04,实跑)**:`02_impl/test/run_all.sh` → **exit 0**,119 秒:

| 环 | 内容 | 结果 |
|---|---|---|
| 0 | 元检查(闸门会不会响)**前置** | 14/14 |
| 1–2 | 严格编译 `-Werror` / 魔数扫描 | PASS |
| 3 | 模块自验 | **47/47** |
| 3b | 负编译(含 N8 单位守卫) | 11/11 |
| 3c | 变异自证(**每个变异两条断言**) | **39/39** |
| 4 | 杀伤矩阵 | **21/21 全杀** |
| 5–6 | 第二轨 bit-exact / 强类型中立性 | PASS |

`01_design/fixedpoint/run_r3.sh` → **exit 0**(含新增 CHK-5g/5g+)。
