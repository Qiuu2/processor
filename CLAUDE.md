...
- 阶段:骨架+W0 技术雷达(D0)+ 竞品解剖(D0b)+ NHS 定向调查(D0c)就绪。**DSP 平台已定型:ADSP-21569** `[已过门 DEC-0006]`(SHARC+ 单核,定点口径;信息卡 `00_input/DSP_ADSP21569_infocard.md`,平台料入 `knowledge_base/adsp21569/`;**算力记账口径 = 按模块分档制** `[未过门 · DEC-0009 自身标着待 CTO 追认]`,**旧「30-50 cyc/MAC 全局包络」已作废、不得并存**——
  > **⛔⛔ 本行原写「T1 已获厂家锚点:EE-408 载明 … 0.5 cyc/MAC [L2/厂家],风险方向较原判反转(DEC-0014)」—— 该句【已随 DEC-0023 撤回】,⛔ 不得再引用。**
  > **理由:那个 `2.5 cyc/biquad` 是【浮点】数,而本项目锁定点口径 ⇒ 它不是定点锚点(详见下一条)。**
  > **⚠ 而这句撤回标记是补记的(2026-08-05):它原本【只以下一条"新增"的形式存在】,本行一字未改 —— 而 lead 自上而下读时先撞见本行,据此派错过一次工单。⇒ ⭐ 这就是"改结论时没回去标旧的那句"的实例,留痕于此。**
  ;旧写「敞口 ×4.3 朝更差」**已撤回**——8.51 属**单 MAC 短环异结构**,套用到 biquad 深链正是 DEC-0009 自己禁止的跨结构套用。**计入值 T1=2.0 不动** `[未经评审 · ⚠ 其成文依据已失效]`——**⚠ T1=2.0 的原依据是「相对厂家值 0.5 留 4× 余量」(`W1A:83`),而 0.5 在定点口径下不适用 ⇒ 该余量是对一个不适用的数留的。⇒ 值可能仍合适,但它目前【没有成文依据】,归架构侧 + CTO,⛔ lead 不自决。****⚠ 厂家锚点 ≠ 板上值**(同族前车之鉴:桌面 vs 板上差 ~25×);T2 仍无板证上界)
...
- **⭐ 架构必读(反直觉,靠直觉会做反)**:**IIRA 只有 1 个 MAC 单元、SHARC+ 核心有 2 个 ⇒ IIR 用加速器比用核心慢(0.42×)**;FIRA 相反(0.25 cyc/tap,长 FIR 真加速)。⇒ **biquad 链(陷波/PEQ)走核心,输出链线性相位 FIR 走 FIRA**,IIRA 仅作卸载。
  > **`[未经评审]` ⚠ 而 `0.42×` 与 `0.25 cyc/tap` 两个数【连 L 标都没有】**(critic `ENTRY_FILES` verdict MAJOR-3)**,却直接支撑上面那条走向裁定 —— 而该裁定决定 D3/D4 与 D13 的实现形态。⇒ ⛔ 引用前须补 L 标并过门。**

  **内存**:L1 = 5 Mbit = **640 KB** 且字节寻址(`.ldf` 载明)`[未经评审]`;**C3=16KB 是模块配额不是硬件墙**,越 C3 = 配额谈判(C9)而非物理越界 `[已过门 DEC-0014]`。;上游厂家界面编辑软件待 CTO 提供(到手走铁律六 24h 入库)。
- 所有 skill 均为**薄骨架**(域未跑、无真料),真本事按 Playbook Part 2 蒸馏 SOP 随真跑长入,禁一次性写"完美 skill"。
- **⭐⭐ 门禁标记读法(2026-08-05 新增,起因 critic `ENTRY_FILES` verdict = FAIL/3 MAJOR)**:本节每条实质断言后带 `[已过门 <verdict>] / [未过门] / [未经评审]` 之一。
  > **⛔ 默认值是【未经评审】,⛔ 不留空 —— 留空的默认读法是"没问题",而那正是此前的实际状态:8 条实质断言里只有 1 条带标记,而它有标记只是因为 critic 恰好评审到了它。**
  > **⇒ ⭐ 即:门禁标记此前不是【制度产出】,是【评审的副产品】。没被评审到的就没有。本条改的就是这一点。**
  > **⚠ 而 `[未经评审]` ≠ 错 —— 它只说明【没人独立看过】。⇒ 下游引用前自行补门,⛔ 不得当既成事实用。**
