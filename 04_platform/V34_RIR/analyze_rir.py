#!/usr/bin/env python3
"""✅ 自测【通过】—— 2026-08-03 第五轮。根因不在本文件,在 make_sweep.py 的逆滤波器。

已知真值合成料(T60 = 0.400 s)实测 [L1/自测实测,`python3 _selftest.py`]:
  T60          0.400 s           ✅ 误差 0.0%(换真值复核:0.250→0.249 / 0.800→0.801)
  相位过零     635 个            ✅ 正/负 bin = 10730/10842,相位正常缠绕
以下三项是【免费的交叉验证】(各有可独立算出的真值),同时对上才算这条链真通了:
  σ_dB         5.47 dB           ✅ 扩散场瑞利普适值 5.57
  DRR         −10.69 dB          ✅ 该合成料解析值 −10.45 dB
  各带频响     71–72 dB 全平      ✅ 合成料是白噪 IR,本就该平

── 根因:ESS 逆滤波器的包络【时间方向反了】(make_sweep.py)────────────────
  误  inv = x[::-1] * exp(-t[::-1]*R/T)   包络随索引【上升】⇒ 幅度补偿 ∝ 1/f
  正  inv = x[::-1] * exp(-t*R/T)         包络随索引【下降】⇒ ∝ f(即 ∝ 瞬时频率)
  差 f²:实测组合响应 X·INV 的斜率 = −12.04 dB/oct(正确应为 +0.00)。
  ⇒ 反卷积出的不是冲激,是一坨慢衰减包络:峰后 1 ms 只占峰后 100 ms 能量的 21.7%
    (单位冲激应 ≈100%;修好后 99.95%)⇒ T60 高估 3.5x、相位平滑到不缠绕。
  ⇒ **下游统计量一直是对的,错的是喂进去的 h。前四轮修下游,所以修不好。**

已修好的(勿回退):
  · make_sweep.py:24 逆滤波器包络的时间方向 —— 本轮,根因
  · 截断:改用相对峰值 −60 dB(原用"末段中位+3dB",而末段是数值零 ⇒ 截断形同虚设)
  · NF ≥ len(h)(np.fft.rfft 在 NF<len 时【静默截断】)
  · sign(0) 归一(避免一次过零被算成两次)

⚠ 动过 make_sweep.py 必须重跑它以重生成 inverse_filter.npy。sweep.wav 本身逐字节未变
  ⇒ 用旧 sweep.wav 录的 rec.wav 仍然有效,不必重录。
⚠ 反回归已验:把逆滤波器换回旧写法,_selftest.py 三条判据全 FAIL 并复现原数
  (1.387 s / 0 个 / 0 正 bin),退出码 1 ⇒ 该自测确实依赖被测物,不是假绿。
⚠ 更正旧表述:原文件头写「make_sweep.py 已验证可用」是错的 —— bug 就在它里面。
  它当时只验过"能生成 sweep.wav",没验过逆滤波器。
"""
