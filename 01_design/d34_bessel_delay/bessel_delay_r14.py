...
# ⛔⛔ 整改 2026-08-05(critic 02impl-r11 m-1):归一化口径**承重**,原先只在源码里,表旁没写。
#   同一个 Bessel8@80 在三种归一化下是 6.326 / 1.990 / 12.220 ms —— **跨度 6×**
#   ⇒ 不写归一化,本表任何一格都无法被复现。
#   ⚠ 而 critic 复算时正是因为缺这类限定(它按【高通支】算)而一度准备开「Bessel 复现不出」。
print( "  归一化口径(⛔ 承重,复现必需):"
      "Bessel = −3 dB@fc(bessel_poles_norm 二分求 |H|=1/√2 处并归一)、"
      "BW = −3 dB@fc(固有)、LR = BW² ⇒ −6 dB@fc")
print( "  ⇒ 三型在 fc 处的衰减本就不同;同一 fc 下比 τ 是行业惯例,而必须注明。")
