...
def main():
    ...
    W("⛔⛔⛔ **本批的作用域限定(lead 2026-08-04 裁定,置于全部数字之前)**:")
    W("     **本批 `NN` 恒为 8 ⇒ 只测了带宽预算的【总量】,未测其【分配方式】。**")
    W("     ⇒ 本批走的是「总带宽 1.60 → 1.00 → 0.80 **往下走**」;")
    W("       而决定「要不要 AFC」的是**另一条正交的线**:总带宽钉在 1.00,把它切成**更多更窄**的份")
    W("       (`NN×bw_oct` 等预算线:8×1/8 = 10×1/10 = 16×1/16 = 24×1/24 = 1.00 oct)。")
    W("     ⇒ **⛔ 因此本批【单独】不足以支持任何『该不该合规』的建议,")
    W("       ⛔ 尤其不得据此建议『保持超预算』** —— 合规的出路在那条没跑的线上(r79)。")
    W("")
    W("⛔⛔ **本轮新增的一条,先于全部数字**:")
    ...
    # ── §H Hb2:头条 Z / Y% 对 bw_oct 稳不稳健 ─────────────────
    W("=" * 118)
    W("§H  ⭐⭐ **Hb2 —— 本批最有价值的一问**:上界与自选若【同比例】下降 ⇒ 差距 Z 不变")
    W("     ⇒ 则 r76 的头条(Z / Y%)对那个超预算常数 **稳健**;不同比例 ⇒ 头条随带宽移动。")
    W("     ⚠ 判据用仪器底:|ΔZ| < 0.354 ⇒ 该种子上「Z 未变」;≥ 0.354 ⇒ Z 真的变了。")
    W("=" * 118)
    W(f"{'T60':>5}{'sd':>4}{'Z@1/5':>9}{'Z@1/8':>9}{'ΔZ':>9}"
      f"{'Y%@1/5':>9}{'Y%@1/8':>9}  判定")
    nmove = ntot2 = 0
    for (t60, sd) in SE:
        a, b = K.get(('C1', t60, sd)), K.get(('C2', t60, sd))
        if not (a and b) or not (np.isfinite(a['Z']) and np.isfinite(b['Z'])):
            continue
        dz = b['Z'] - a['Z']
        ntot2 += 1
        moved = abs(dz) >= FLOOR
        if moved:
            nmove += 1
        W(f"{t60:>5.1f}{sd:>4}{a['Z']:>9.2f}{b['Z']:>9.2f}{dz:>+9.2f}"
          f"{a['Y']:>9.0f}{b['Y']:>9.0f}  "
          + ('**Z 真的变了(在仪器底之上)**' if moved else '(ΔZ 在仪器底之下,不可判)'))
    W(f"  ⇒ **Z 在 {nmove}/{ntot2} 条种子上发生了可判的变化**")
    W("  ⇒ 判读(机械):Z 若在多数种子上可判地变化 ⇒ **上界与自选【不同比例】下降**")
    W("    ⇒ 则 r76 的头条 Z / Y% **不是 bw_oct 无关量**,引用时须带 `bw_oct = 1/5` 限定。")
    W("")
