"""r91 · 其余五格的 T_OBS 收敛性(lead 批;r90 的补完)。单格 worker。
⛔ 未经 critic 评审。[L2/宿主仿真]。预注册 = PREREG_r91.txt(§2 判据跑前写死,含"未收敛怎么记")。
输出 r91_cell_<tag>_out.txt(D6-j)。⛔ 本文件不含结论性散文。
"""
...
def main():
    ...
    pairs = [tuple(p) for r in json.load(open(DIR + 'r89_na_bias.json'))
             if r.get('repro') and r['T60'] == T60 and r['sd'] == SD for p in r['pairs']]
    W(f"未经 critic 评审 —— r91 单格 tag={A.tag}(T60={T60}/sd={SD})  [L2/宿主仿真]  预注册 = PREREG_r91.txt")
    W(f"⭐ 解析真值**本轮自算**(MSGMeter),⛔ 不从 critic verdict 转抄")
    W(f"冻结 Na 配置(取自 r89):{pairs}")
    ...
    mN_ana = mt.msg(slots=mk_frozen(pairs).slots, g_duck_db=0.)['full']['msg_db']
    ...
    for T in RUNGS:
        ...
        mN = scan(hb, D, lambda _p=pairs: mk_frozen(_p), anchor - 1, anchor + 20, src, ref)
    ...
    open(DIR + f'r91_cell_{A.tag}_out.txt', 'w').write("\n".join(OUT) + "\n")
    json.dump(rows, open(DIR + f'r91_cell_{A.tag}.json', 'w'))
