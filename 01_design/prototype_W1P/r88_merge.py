"""r88 归并。⛔ 未经 critic 评审。[L2/宿主仿真]。预注册 = PREREG_r88.txt。
...
def main():
    R = []
    for p in sorted(glob.glob(DIR + 'r88_cell_*.json')):
        R += json.load(open(p))
    K = {(r['T60'], r['sd'], r['plant']): r for r in R}
    W("未经 critic 评审 —— r88 · **非统计 plant 对照**  [L2/宿主仿真]  预注册 = PREREG_r88.txt")
    ...
    W("=" * 116)
    W(f"{'T60/sd':>8}{'P_stat':>9}{'P_conf':>9}{'P_booth':>10}"
      f"{'δ=conf−stat':>13}{'判读(主问)':>22}{'δ=booth−stat':>14}{'判读(阳性对照)':>24}")
    dc, db = [], []
    for (t, s) in SEEDS:
        a, b, c = K.get((t, s, 'P_stat')), K.get((t, s, 'P_conf')), K.get((t, s, 'P_booth'))
    ...
    W(f"   Hp2 阳性对照:P_booth vs P_stat **可判 {nb}/{len(db)}**")
    W(f"   ⇒ 机械判读(按预注册 §4):")
    if nb == 0:
    ...
    nb2 = tot = 0
    for (t, s) in SEEDS:
        for nm in ('P_stat', 'P_conf', 'P_booth'):
            r = K.get((t, s, nm))
            if not r:
                continue
            tot += 1
    ...
    with open(DIR + 'r88_plant_out.txt', 'w') as fp:
        fp.write("\n".join(OUT) + "\n")
    json.dump(R, open(DIR + 'r88_plant.json', 'w'), default=lambda o: None)
