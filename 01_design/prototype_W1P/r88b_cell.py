...
⛔ 未经 critic 评审。[L2/宿主仿真]。预注册 = PREREG_r88b.txt。
...
L_CONF, L_MOD9 = (5.8, 4.6, 3.75), (2.2, 2.1, 1.95)   # V=100 / V=9.01(阳性对照,全段 M<1)
OUT = []
...
def main():
    ...
    W(f"未经 critic 评审 —— r88 单格 tag={A.tag}  [L2/宿主仿真]  预注册 = PREREG_r88b.txt")
    ...
    W(f"plant:P_stat=clrig 噪声RIR / P_conf=模态 {L_CONF} (100 m³) / P_mod9=模态 {L_MOD9} (9.01 m³,**阳性对照,全段 M<1**;仅 T60=0.5)")
    ...
    if abs(T60 - 0.5) < 1e-9:          # 阳性对照只在 T60=0.5 上存在(T60=0.2 需 V<3.7 m³)
        plants.append(('P_mod9', lambda: modal_rig.make_F_modal(T60=T60, prop_delay_ms=8., seed=sd, L=L_MOD9)))
    rows = []
    ...
    with open(DIR + f'r88b_cell_{A.tag}_out.txt', 'w') as fp:
        fp.write("\n".join(OUT) + "\n")
    with open(DIR + f'r88b_cell_{A.tag}.json', 'w') as fp:
