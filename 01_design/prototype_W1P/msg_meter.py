...
**⚠⚠ nfft 不得为了跑得快调小 —— 粗网格的误差【方向是偏乐观的】**
  实测收敛(r53 §C,T60=0.5/seed2/8陷波,最难一档):
      2^15 −7.163 / 2^16 −7.332 / 2^17 −7.422 / **2^18 −7.4239** / 2^19 −7.4249 / 2^20 −7.4254
  ⇒ 2^15 的偏差 **+0.26 dB,且系统性偏高(= MSG 更好)**;临界点越密偏差越大 ⇒ **T60 越大越差**。
  ⇒ **调小 nfft 会把数值误差读成算法收益**,而且是往"我们希望的方向"错。
  ⇒ 默认 2^18 是实测收敛点(单次 0.035 s),不是拍的。要改先重跑收敛表。
