...
print("r16 · 长窗 100s 同种子双臂 —— 弃权代价是否被 LIFT 吸收")
...
print(f"{'类':<12}{'臂':<14}{'末挂陷@6s':>11}{'窗末累积占用@100s':>14}"
      f"{'弃权数':>8}{'弃权占用':>9}{'其中经LIFT':>11}{'回收比例':>9}{'LIFT观测':>9}")

for nm, mk in [('钢琴', S.m_piano), ('裸纯音停', S.m_bare_stop)]:
    store = {}
    for on in (True, False):
        acc = dict(h6=0, hend=0, ab=0, ab_occ=0, ab_lift=0, lo=0)
        for i in range(N):
            r = run_long(mk, i, on)
        ...
        print(f"{nm:<12}{lab:<14}{acc['h6']/N:>11.2f}{acc['hend']/N:>14.2f}"
              f"{acc['ab']:>8}{acc['ab_occ']:>9}{acc['ab_lift']:>11}{rs:>9}{acc['lo']:>9}")
        sys.stdout.flush()
